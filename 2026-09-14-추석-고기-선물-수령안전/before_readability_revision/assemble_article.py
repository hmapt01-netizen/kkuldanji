...
for c,label in zip(byclass(g,'info-section-card'),['시간 이력','수령 온도','표시·포장']):
...
data=dict(title=gt,shortTitle='추석 고기 택배 수령과 해동 후 보관',date='2026.09.14',category='식단 & 영양',
 ...
 desc='추석 고기 택배를 상온에 하루 두었다면 냉장고에 넣기 전 시간 이력, 식품 온도, 표시·포장을 확인하세요. 녹은 아이스팩과 고기의 상태를 구분하고 안전한 냉장 해동의 재냉동 조건을 살펴봅니다.',
 thumb=f'images/posts/{SLUG}/thumb.jpg',featuredCaption='선물 보냉 상자와 밀봉 고기, 녹은 아이스팩',
 isLatest=True,isEditorPick=False,academicSource='식약처·FDA·USDA FSIS·FoodSafety.gov 공공 안내 기반',
...
paras=list(n.iter('p'))
storage=next(p for p in paras if textof(p).startswith('정상 수령한 고기는'))
storage.addnext(fragment(picture('post04.jpg','naver')))
after=next(p for p in paras if textof(p).startswith('이번 추석에는'))
...
underline=next(p for p in paras if textof(p).startswith('늦게 식혔다고'))
...
for text,name in [('정상 수령한 고기는 표시대로 바로 보관하고,\n육즙이 다른 음식에 닿지 않게 분리해 주세요.','post04.jpg'),('이번 추석에는 선물 보내기 전 연락할 때\n“언제 집에서 받을 수 있어요?”를 덧붙여 보세요.','post05.jpg')]:
